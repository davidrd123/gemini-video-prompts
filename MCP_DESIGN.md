...
|------|---------------|---------|-------------|
| `describe_image` | `gemini-3.8-flash` | Rich structured observations of one image; no scores | **Claude** — consumes the description, applies the rubric herself |
| `score_image` | `gemini-3.8-flash` | 6-dim scored eval of one image; advisory `decision_hint` | **Gemini** — Claude can override |
| `describe_video` | `gemini-3.8-flash` | Rich structured observations of one video; video-aware fields | **Claude** |
| `score_video` | `gemini-3.8-flash` | 6-dim scored eval of one video, dims adapted per skill SKILL.md:290-300 | **Gemini** |
| `analyze_video` | `gemini-3.8-flash` | Free-form question over one video; defaults to high thinking and 65,536 max output tokens | **Claude** |
| `analyze_videos` | `gemini-3.8-flash` | Free-form cross-video question over 2–10 ordered, labeled videos; defaults to high thinking and 65,536 max output tokens | **Claude** |
| `analyze_audio` | `gemini-3.8-flash` | Free-form auditory analysis or transcription of one recording | **Claude** |
| `compare_images` | `gemini-3.8-flash` | "Which is better"; pick + reasoning | **Gemini** |
| `extract_visual_tokens` | `gemini-3.8-flash` | Categorized token deconstruct for env-coverage genesis workflow | **Gemini (descriptive)** |
...
| 14 | Default model for analysis tools | **Closed** — `gemini-3.8-flash` for image, video, and audio tools |
...
download. The provenance extension adds normalized requested/effective seed
tracking, SHA-256 identities from the opened upload streams and downloaded
outputs, an append-only `index.ndjson` with a shared write lock and recoverable
...
- `get_generation(job_id)` - bundles the immutable request, latest status, final
  result, record paths, and seed provenance without polling.
- `list_generations(...)` - searches indexed and historical local job records.
